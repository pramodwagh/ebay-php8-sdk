<?php
namespace PWCode\eBaySDK\Types;

use \PWCode\eBaySDK\Types;
use \PWCode\eBaySDK\Exceptions;
use \PWCode\eBaySDK\JmesPath\Env;
use \PWCode\eBaySDK\JmesPath\JmesPathableObjectInterface;
...
class BaseType implements JmesPathableObjectInterface
{
     ...
     *
     * @throws \PWCode\eBaySDK\Exceptions\UnknownPropertyException If the property does not exist.
     * @throws \PWCode\eBaySDK\Exceptions\InvalidPropertyTypeException If the value is the wrong type for the property.
     */
     ...
     * @param mixed $value. The value to assign to the property.
     *
     * @throws \PWCode\eBaySDK\Exceptions\InvalidPropertyTypeException If trying to assign a non array type to an repeatable property.
     */
    private function setValue($class, $name, $value)
    {
        ...
        if (!$info['repeatable']) {
            ...
            if ('array' !== $actualType) {
                throw new Exceptions\InvalidPropertyTypeException($name, 'PWCode\eBaySDK\Types\RepeatableType', $actualType);
            } else {
    ...
    private static function ensurePropertyType($class, $name, $value)
    {
        ...
        foreach ($valid as $check) {
            if ($check !== 'any' && \PWCode\eBaySDK\checkPropertyType($check)) {
    ...
    {
        if (is_subclass_of($value, '\PWCode\eBaySDK\Types\BaseType', false)) {
