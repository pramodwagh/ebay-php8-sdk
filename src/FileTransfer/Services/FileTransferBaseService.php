 ...
 */
class FileTransferBaseService extends \PWCode\eBaySDK\Services\BaseService
{
    /**
    ...
    public static function getConfigDefinitions()
    {
        ...
        return $definitions + [
            'apiVersion' => [
                'valid' => ['string'],
                'default' => \PWCode\eBaySDK\FileTransfer\Services\FileTransferService::API_VERSION
            ],
            'authToken' => [
                'valid' => ['string'],
                'required' => true
    ...
    protected function callOperationAsync($name, \PWCode\eBaySDK\Types\BaseType $request, $responseClass)
    {
        ...
        if ($name === 'uploadFile' && $request->hasAttachment()) {
            ...
            if (!isset($request->fileAttachment)) {
                $request->fileAttachment = new \PWCode\eBaySDK\FileTransfer\Types\FileAttachment();
            }

            if (!isset($request->fileAttachment->Data)) {
                $request->fileAttachment->Data = new \PWCode\eBaySDK\FileTransfer\Types\Data([
                    'xopInclude' => new \PWCode\eBaySDK\FileTransfer\Types\XopInclude([
