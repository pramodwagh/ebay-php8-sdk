...
.. code-block:: php
    ...
    use \PWCode\eBaySDK\Finding\Types;
...
Properties of the objects are named after the fields found in the offical eBay documentation. The SDK will throw a ``PWCode\eBaySDK\Exceptions\UnknownPropertyException`` if you get or set a property that does not exist.
...
A ``PWCode\eBaySDK\Exceptions\InvalidPropertyTypeException`` is thrown if a value of the wrong type is assigned to a property.
...
You can disable this type checking by assigning ``false`` to the static property ``\PWCode\eBaySDK\Sdk::$STRICT_PROPERTY_TYPES``.

.. code-block:: php

  \PWCode\eBaySDK\Sdk::$STRICT_PROPERTY_TYPES = false;
