...
The ``PWCode\eBaySDK\MerchantData\Services\MerchantDataService`` class was removed in version 1 and replaced with the ``PWCode\eBaySDK\MerchantData\MerchantData`` class. There is no difference in how the new class works when compared with the previous version.

Removed UnboundType class
~~~~~~~~~~~~~~~~~~~~~~~~~

The ``PWCode\eBaySDK\Types\UnboundType`` class was removed in version 1 and replaced with the ``PWCode\eBaySDK\Types\RepeatableType`` class. There is no difference in how the new class works when compared with the previous version.
...
.. code-block:: php
    ...
    use PWCode\eBaySDK\Trading\Services\TradingService;
...
The pre-version 1 SDK would throw a ``PWCode\eBaySDK\Exceptions\UnknownConfigurationOptionException`` when a non-existent configuration option was passed to either the service constructor or ``config`` method. The SDK will now silently ignore any unknown configuration options.
...
.. code-block:: php
    ...
    } catch (\PWCode\eBaySDK\Exceptions\UnknownConfigurationOptionException $e) {
        echo $e->getMessage();
    }
...
.. code-block:: php
    ...
    // Pre-version 1
    class HttpClient implements \PWCode\eBaySDK\Interfaces\HttpClientInterface
    {
