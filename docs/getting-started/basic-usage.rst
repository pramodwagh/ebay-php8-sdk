...
.. code-block:: php
    ...
    $finding = new PWCode\eBaySDK\Finding\Services\FindingService([
        'apiVersion' => '1.13.0',
        'globalId'   => PWCode\eBaySDK\Constants\GlobalIds::US
    ]);
...
The ``PWCode\eBaySDK\Sdk`` class can be used as a factory for creating services. Configuration options passed to this class are shared with any services that it creates.
...
.. code-block:: php
    ...
    $sdk = new PWCode\eBaySDK\Sdk($config);
...
Properties of the request object can then be assigned values that will be sent to the API. Note that you may have to create instances of other classes, such as `PWCode\\eBaySDK\\Finding\\Types\\PaginationInput <https://github.com/daviPWCodeadler/ebay-sdk-php/blob/master/src/Finding/Types/PaginationInput.php>`_, in order to build up a complete request.
